...
> 최종 업데이트: 2026-09-13 · 상태: **적성·장비·시작 캐릭터·획득 경로(가챠)·경험치 획득·레벨 곡선 확정 · 레벨 효과 미정**
...
>   [`진행 및 성장`](../progression/README.md) · [`특성`](../trait/README.md)
...
(② 계정 레벨과 함께 충족해야 한다) → [산업 레벨](../gathering/산업레벨.md) 3장.

- **배치하지 않아도 된다.** 갖고 있기만 하면 조건을 만족한다.
- 그래서 적성은 **"이 슬롯이 얼마나 빠른가"에 더해 "어떤 콘텐츠가 열리는가"** 를 함께 정한다.

> ⚠️ **획득 경로는 가챠로 확정됐지만(5.1) 천장·확정 지급은 아직 없다.**
> 적성 N짜리 캐릭터를 얻는 길이 가챠뿐이므로,
> 운이 나쁜 유저는 산업 레벨이 영영 열리지 않는다.
> **보조 경로(상점 구매·퀘스트 확정 지급·천장)는 남은 미정이다.**
...
> 적성 0인 산업을 가진 캐릭터가 실제로 손에 들어와야 **배치 화면의 잠금 표시**와
> **산업 레벨 해금 조건 ①**(2.3)이 화면에서 검증된다.
...
| 4 | **가챠 천장 · 확정 지급 경로** | 뽑기만으로는 적성 N을 보장하지 못한다 → 2.3 · 5.1 |
