...
> 상태: **3열 배치 · 연출 2레이어 · 위젯 배치·스트립 방향 확정 / 목업 1종(v2)**
> **바뀌면 갱신:** [`게임기획코어`](../게임기획코어.md) · [`우편`](../mail/README.md) · [`캐릭터`](../character/README.md) · [`퀘스트`](../quest/README.md)
...
**2026-08-01 추가 확정 — [2026-07-30 방향 제안](../proposals/2026-07-30/2026-07-30-종합제안정리.md) 합의분.**
...
| **상점의 정체** | **골드 상점.** 현금 상품은 두지 않는다 (BM은 여전히 미정) | 제안 4.4 · [거래](../trade/README.md) 3.2 |
...
> [`proposals/2026-07-30/`](../proposals/2026-07-30/)에 기록으로 남아 있다.
...
> 골드 상점이라는 방향만 잡혔고 **수치는 전부 미정**이다. → [거래](../trade/README.md) 3.2

> ⚠️ **v2에도 산업 레벨이 반영돼 있지 않다.**
> 배치가 `(산업, 캐릭터)` 두 칸으로 그려져 있는데, 실제로는 **레벨이 한 칸 더 붙는다.**
> 패킷 레벨 필드는 붙었다(T-017 완료 — `C_WorkStationAssignRequest.IndustryLevel`).
> 해금은 [해금](../unlock/README.md)으로 통일됐다(2026-09-14) — 잠긴 것은 전부 보이고 조건도 보인다. 계정 레벨 요구치(T-016)가 정해지면 목업을 갱신한다.
...
![3열 배치 레퍼런스 — 테스크바 히어로](references/UI%20배치%28테스크바%20히어로%29.png)
...
> 원본 근거: [방향 제안](../proposals/2026-07-30/2026-07-30-종합제안정리.md) 2장.
...
![위젯 위치 선택 UI — 테스크바 히어로](references/위젯%20UI%20배치%20선택%28테스크바%20히어로%29.png)
...
| 들어올 후보 | 근거 |
| --- | --- |
| **보스** | 사냥에서 티켓을 모아 도전. 채취 루프 밖으로 나가는 **유일한 통로** → [사냥](../gathering/hunting/README.md) 4장 |
...
> ⚠️ **여기 들어오는 것은 무엇이든 [사냥](../gathering/hunting/README.md) 4.2의 보스 4원칙을 지킨다** —
> 도전 시점 자유 · 실시간 조작 없음 · 실패해도 티켓만 손실 · **기간 한정 없음**.
>
> **버튼에 배지·타이머·"오늘까지" 표시를 붙이지 않는다.**
> (예외는 [우편](../mail/README.md)함 버튼의 **점 하나** — 숫자·기한이 없고, 기다리는 것이 사라지지 않는 보상이다)
...
**구현: 일감 [T-015](../../../tasks/art/T-015-산업아이템클라적용.md)**
...
| 절전·최대 절전 | 복귀 시 경과 시간 정산 → [자원채취](../gathering/README.md) 2.2 | ❌ 미결 |
...
| 레퍼런스 | **테스크바 히어로** — 창 프레임·아이콘 격자·위치 선택 UI ([참고 사진](references)) |
...
7. ~~**상시 스트립의 방향은?**~~ ✅ **해소 (2026-08-26) — 가로.**
   위젯이 창 안 6칸 중 한 칸을 **가로로 길게**(폭 633 × 높이 87) 차지하므로,
   같은 자리에 세로 스트립을 세우면 칸 크기가 87px 안에 갇힌다.
   ⚠️ **폭이 늘어나는 단점은 그대로 안고 간다** — 슬롯 8개면 칸(49) + 간격(4)으로 약 420px를 쓴다.
   위젯 폭(633)에는 들어가지만 **슬롯이 8을 넘으면 다시 봐야 한다.**
...
9. ~~**상점은 골드 상점인가 현금 상점인가?**~~ ✅ **해소 (2026-08-01) — 골드 상점** (1장).
   품목은 미정이다 — **작업슬롯 확장권은 상점 품목이 아니다** (2026-09-16 · 잠긴 칸에서 직접 연다 → [해금](../unlock/README.md) 4.1).
   ⚠️ **가챠는 상점 품목이 아니다** — 골드를 직접 내고 뽑는 별도 화면이다 → [캐릭터](../character/README.md) 5.1.
   ⚠️ 슬롯 해금은 잠긴 칸의 조건 표시(골드·선행·계정 레벨)가 담당한다 → [T-039](../../../tasks/art/T-039-클라슬롯해금UI.md).
10. 해금하지 않은 산업 레벨은 **보여 준다** — 조건과 함께 ✅ (2026-09-14 → [해금](../unlock/README.md) 7장).
    2.4의 "비어 있을 때는 잠긴 표시를 보여 주지 않는다"는 **기획이 없는 콘텐츠 자리**에 대한 규칙이고,
    해금 대상(슬롯·산업 레벨·특성)은 전부 보인다.
...
12. ~~**작업슬롯 캔버스 하단 버튼 3~5번에 무엇이 들어가는가?**~~ ✅ **해소 (2026-09-16) — 2.4의 특별 이벤트 자리와 같은 줄이다.**
    별도 층을 두지 않는다. `창고`·`거래` 옆 빈 자리에 채취 루프 밖의 콘텐츠(보스 · 대형 작업물)가 들어오고,
    줄 높이가 고정이라 버튼이 늘어도 작업슬롯 영역은 밀리지 않는다.
    **무엇이 들어가는지는 여전히 미기획이다** — 2.4의 후보 표를 따른다.
14. **적성 포인트를 어디서 찍는가?** — [캐릭터](../character/README.md) 5.4가 화면 요구를 정했다(산업별 적성·상한·남은 포인트·다음 포인트까지 레벨, 찍기 전 확인 1회).
    창고의 캐릭터 상세인지, 배치 선택창의 캐릭터 줄인지는 미정. 서버 패킷(`C_AptitudeUpRequest`)은 T-003이 만든다.
