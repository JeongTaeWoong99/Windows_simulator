...
> 최종 업데이트: 2026-08-14 (트리거 범위를 "게임 규칙·수치가 바뀌는 작업"으로 한정)
>
...
### 바뀐 내용만 적는다 — 이력을 남기지 않는다

**기획이 달라지면 이전 서술을 지우고 새 내용으로 대체한다.**
취소선(`~~…~~`)·"폐지"·"재정의됨"·"해소" 같은 **변경 이력을 문서에 남기지 않는다.**
문서는 **지금 확정된 상태**만 보여준다 — 변경 이력은 git이 갖는다.

- 폐지된 시스템은 항목째 **삭제한다.** 그 결과 지금 유효한 규칙이 있으면
  (예: "요일 보너스 없음") **긍정문 한 줄로** 남긴다.
- `게임기획코어.md` 5장의 확정/미확정 표도 같다 — `❌ 폐지` 행을 쌓지 않는다.
