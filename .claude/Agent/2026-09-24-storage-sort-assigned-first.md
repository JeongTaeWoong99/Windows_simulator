...
## 업데이트 4 — **팰월드 방식도 철회됐다** (2026-09-25)

위 "업데이트 3"의 결론(**목록에서 뺀다**)은 **같은 날 되돌려졌다.**
최종 사양은 **제자리에 남기고 딤 + `배` 마크 + [정렬] 맨 뒤**다.
경위·근거는 [2026-09-25 로그](2026-09-25-storage-keep-deployed-in-place.md)에 있다 —
이 문서의 업데이트 3 이하 내용은 **더 이상 현재 설계가 아니다.**

다만 "표시를 세 번 고쳐도 안 풀리면 판정을 의심한다"는 교훈 자체는 남긴다.
네 번째 답도 결국 판정(`IsAway`)을 한곳으로 모아서 풀었다.
