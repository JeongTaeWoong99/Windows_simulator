...
- [2026-09-07 핑 송신을 백그라운드 타이머로 — 드래그 끊김의 클라 몫](2026-09-07-ping-timer-client.md) — `#client` `#network` 넣는 쪽(타이머)과 비우는 쪽(B-1)이 **둘 다** 메인 스레드 밖이어야 한다 · 🔑 오탐 가드가 없으면 서버가 안 끊어도 **클라가 스스로 앱을 내린다** · ✅ 빌드 20초 드래그 실측 통과
