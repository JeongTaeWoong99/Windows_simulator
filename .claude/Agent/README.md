...
- [2026-08-14 에디터 서버 콘솔 — 버튼으로 WSGameServer 토글 실행 + 로그 tail](2026-08-14-editor-server-console.md) — 상단 툴바 '서버 콘솔' 버튼 → 창에서 시작/정지 토글 + 로그 실시간 보기 · `Editor/`를 `shared`·`scene-copy`·`server-console` 하위 폴더로 재편(`.cs`+`.meta` git mv) · **로그를 `cmd > Temp/WSGameServer.log 2>&1`로 OS 리다이렉트** — in-process stdout 캡처는 도메인 리로드마다 끊기므로 파일 tail로 우회(`chcp 65001`+UTF-8로 한글 유지) · PID는 `SessionState`(리로드 넘어 생존, 재시작 때 소멸)로 재부착 · 종료는 `taskkill /T`로 cmd→dotnet→WSGameServer 트리 전체 · ⚠️ **Unity 실구동·`.meta` 생성 미검증**(새 폴더/스크립트 6개 meta는 에디터 갱신 필요)
