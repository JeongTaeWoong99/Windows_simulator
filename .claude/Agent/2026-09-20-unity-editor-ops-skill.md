...
## 업데이트 (2026-09-22) — 2절 `임의 C# 실행` 행의 적용 범위 조정

씬·프리팹 편집은 등록된 명령으로 안 되는 경우가 많아 대부분 `eval`·`run_script`로
간다. 그래서 이 행이 사실상 모든 에디터 작업에 걸렸고, 사용자가 확인 요청이
과도하다고 지적했다.

- 행 이름을 **`되돌릴 수 없는 C# 실행`** 으로 바꿔, 안에서 삭제·파괴를 할 때만
  해당되게 했다.
- 판단 표를 소절로 추가했다 — 생성·수정·조회·저장 / 삭제·파괴 / 플레이·빌드·베이크.
- 프로젝트 사본과 마스터에 동일 적용(마스터는 도구 이름 중립 표현).

**`settings.local.json`의 `"ask"` 목록 28줄은 그대로 남아 있다.** auto 모드에서는
도달하지 않지만 모드를 바꾸면 뜬다. 권한 규칙은 사용자가 직접 정리해야 한다.
(gitignore라 커밋 대상 아님)
