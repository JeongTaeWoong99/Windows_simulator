...
### 넘침 가드의 오탐 — 부모에 레이아웃 그룹이 있을 때만 센다

새 가드가 에디터에서 스크롤바 2곳을 상시로 찍어 댔다. **씬 문제가 아니라 가드 문제였다.**

유니티 기본 `Scrollbar`는 `Sliding Area`를 핸들 크기만큼 줄여(`sizeDelta` −20 → **폭 0**)
핸들의 이동 범위를 만들고, `Handle`이 `sizeDelta` +20으로 그만큼 되돌린다. **설계상 정상**이다.
씬의 스크롤바 6개가 전부 기본값 그대로임을 실측으로 확인했다 — **씬은 손댈 것이 없다.**

→ `CollectOverflow`가 **부모에 `LayoutGroup`이 있을 때만** 넘침으로 친다.
"부모 폭이 자식의 상한"이라는 전제는 그 경우에만 성립한다.
A-1 사고 경로(`@Market Column` → `#Market Canvas` → `Gacha Presenter`)는 부모가 둘 다
`VerticalLayoutGroup`이라 **이 필터로도 그대로 잡힌다**(실측 확인).

> 고칠 것이 없는 경고를 계속 내보내면 다음 진짜 경고를 아무도 안 본다.
> 가드는 **오탐이 없어야 가드다.**
