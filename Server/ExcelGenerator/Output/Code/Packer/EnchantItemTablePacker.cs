...
namespace GameData
{
    /// <summary>EnchantItemTable 시트의 셀 문자열을 EnchantItemTableRow로 변환하고 MemoryPack으로 직렬화한다. (패커 전용 — 클라/서버 배포 대상 아님)</summary>
    public static class EnchantItemTablePacker
    {
        private const string Table = "EnchantItemTable";
        ...
        public static EnchantItemTableRow Parse(string[] cells) => new()
        {
            ...
            Action          = PackerUtil.ParseEnum<EnchantAction>(cells[1], Table, "Action"),
            SuccessPermille = PackerUtil.ParseInt(cells[2], Table, "SuccessPermille", 1, 1000),
        ...
        {
            var list = new List<EnchantItemTableRow>(rows.Count);
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<EnchantItemTableRow>>(bytes)
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<EnchantItemTableRow>>(bytes) ?? new();
