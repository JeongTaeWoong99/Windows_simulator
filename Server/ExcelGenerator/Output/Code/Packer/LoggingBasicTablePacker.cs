...
namespace GameData
{
    /// <summary>LoggingBasicTable 시트의 셀 문자열을 LoggingBasicTableRow로 변환하고 MemoryPack으로 직렬화한다. (패커 전용 — 클라/서버 배포 대상 아님)</summary>
    public static class LoggingBasicTablePacker
    {
        private const string Table = "LoggingBasicTable";

        /// <summary>셀 배열(파싱된 컬럼 순서)을 강타입 Row로 변환한다.</summary>
        public static LoggingBasicTableRow Parse(string[] cells) => new()
        {
        ...
        {
            var list = new List<LoggingBasicTableRow>(rows.Count);
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<LoggingBasicTableRow>>(bytes)
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<LoggingBasicTableRow>>(bytes) ?? new();
