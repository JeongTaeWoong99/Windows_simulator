...
namespace GameData
{
    /// <summary>WorkSpeedTable 시트의 셀 문자열을 WorkSpeedTableRow로 변환하고 MemoryPack으로 직렬화한다. (패커 전용 — 클라/서버 배포 대상 아님)</summary>
    public static class WorkSpeedTablePacker
    {
        private const string Table = "WorkSpeedTable";
        ...
        {
            var list = new List<WorkSpeedTableRow>(rows.Count);
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<WorkSpeedTableRow>>(bytes)
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<WorkSpeedTableRow>>(bytes) ?? new();
