...
namespace GameData
{
    /// <summary>MiningBasicTable 시트의 셀 문자열을 MiningBasicTableRow로 변환하고 MemoryPack으로 직렬화한다. (패커 전용 — 클라/서버 배포 대상 아님)</summary>
    public static class MiningBasicTablePacker
    {
        private const string Table = "MiningBasicTable";

        /// <summary>셀 배열(파싱된 컬럼 순서)을 강타입 Row로 변환한다.</summary>
        public static MiningBasicTableRow Parse(string[] cells) => new()
        {
        ...
        {
            var list = new List<MiningBasicTableRow>(rows.Count);
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<MiningBasicTableRow>>(bytes)
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<MiningBasicTableRow>>(bytes) ?? new();
