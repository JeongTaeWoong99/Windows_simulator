...
namespace GameData
{
    /// <summary>FishingBasicTable 시트의 셀 문자열을 FishingBasicTableRow로 변환하고 MemoryPack으로 직렬화한다. (패커 전용 — 클라/서버 배포 대상 아님)</summary>
    public static class FishingBasicTablePacker
    {
        private const string Table = "FishingBasicTable";
        ...
        {
            var list = new List<FishingBasicTableRow>(rows.Count);
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<FishingBasicTableRow>>(bytes)
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<FishingBasicTableRow>>(bytes) ?? new();
