...
namespace GameData
{
    /// <summary>WorkSlotTable 시트의 셀 문자열을 WorkSlotTableRow로 변환하고 MemoryPack으로 직렬화한다. (패커 전용 — 클라/서버 배포 대상 아님)</summary>
    public static class WorkSlotTablePacker
    {
        private const string Table = "WorkSlotTable";

        /// <summary>셀 배열(파싱된 컬럼 순서)을 강타입 Row로 변환한다.</summary>
        public static WorkSlotTableRow Parse(string[] cells) => new()
        {
            WorkSlotTID = PackerUtil.ParseInt(cells[0], Table, "WorkSlotTID", 0, null),
            UnlockTID   = PackerUtil.ParseInt(cells[1], Table, "UnlockTID", 0, null),
        ...
        {
            var list = new List<WorkSlotTableRow>(rows.Count);
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<WorkSlotTableRow>>(bytes)
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<WorkSlotTableRow>>(bytes) ?? new();
