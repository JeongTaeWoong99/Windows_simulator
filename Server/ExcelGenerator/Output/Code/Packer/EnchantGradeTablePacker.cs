...
namespace GameData
{
    /// <summary>EnchantGradeTable 시트의 셀 문자열을 EnchantGradeTableRow로 변환하고 MemoryPack으로 직렬화한다. (패커 전용 — 클라/서버 배포 대상 아님)</summary>
    public static class EnchantGradeTablePacker
    {
        private const string Table = "EnchantGradeTable";
        ...
        {
            var list = new List<EnchantGradeTableRow>(rows.Count);
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<EnchantGradeTableRow>>(bytes)
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<EnchantGradeTableRow>>(bytes) ?? new();
