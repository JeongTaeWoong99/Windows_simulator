...
namespace GameData
{
    /// <summary>ConstantsTable 시트의 셀 문자열을 ConstantsTableRow로 변환하고 MemoryPack으로 직렬화한다. (패커 전용 — 클라/서버 배포 대상 아님)</summary>
    public static class ConstantsTablePacker
    {
        private const string Table = "ConstantsTable";

        /// <summary>셀 배열(파싱된 컬럼 순서)을 강타입 Row로 변환한다.</summary>
        public static ConstantsTableRow Parse(string[] cells) => new()
        {
            Name        = PackerUtil.RequireString(cells[0], Table, "Name"),
            Value       = PackerUtil.ParseLong(cells[1], Table, "Value"),
        ...
        {
            var list = new List<ConstantsTableRow>(rows.Count);
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<ConstantsTableRow>>(bytes)
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<ConstantsTableRow>>(bytes) ?? new();
