...
namespace GameData
{
    /// <summary>AccountLevelTable 시트의 셀 문자열을 AccountLevelTableRow로 변환하고 MemoryPack으로 직렬화한다. (패커 전용 — 클라/서버 배포 대상 아님)</summary>
    public static class AccountLevelTablePacker
    {
        private const string Table = "AccountLevelTable";

        /// <summary>셀 배열(파싱된 컬럼 순서)을 강타입 Row로 변환한다.</summary>
        public static AccountLevelTableRow Parse(string[] cells) => new()
        {
            AccountLevelTID = PackerUtil.ParseInt(cells[0], Table, "AccountLevelTID", 1, null),
            RequiredExp     = PackerUtil.ParseLong(cells[1], Table, "RequiredExp", 0, null),
            TraitPoint      = PackerUtil.ParseInt(cells[2], Table, "TraitPoint", 0, null),
        ...
        {
            var list = new List<AccountLevelTableRow>(rows.Count);
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<AccountLevelTableRow>>(bytes)
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<AccountLevelTableRow>>(bytes) ?? new();
