...
namespace GameData
{
    /// <summary>GachaItemTable 시트의 셀 문자열을 GachaItemTableRow로 변환하고 MemoryPack으로 직렬화한다. (패커 전용 — 클라/서버 배포 대상 아님)</summary>
    public static class GachaItemTablePacker
    {
        private const string Table = "GachaItemTable";

        /// <summary>셀 배열(파싱된 컬럼 순서)을 강타입 Row로 변환한다.</summary>
        public static GachaItemTableRow Parse(string[] cells) => new()
        {
            GachaItemTID = PackerUtil.ParseInt(cells[0], Table, "GachaItemTID"),
        ...
        {
            var list = new List<GachaItemTableRow>(rows.Count);
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<GachaItemTableRow>>(bytes)
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<GachaItemTableRow>>(bytes) ?? new();
