...
namespace GameData
{
    /// <summary>MailTemplateTable 시트의 셀 문자열을 MailTemplateTableRow로 변환하고 MemoryPack으로 직렬화한다. (패커 전용 — 클라/서버 배포 대상 아님)</summary>
    public static class MailTemplateTablePacker
    {
        private const string Table = "MailTemplateTable";

        /// <summary>셀 배열(파싱된 컬럼 순서)을 강타입 Row로 변환한다.</summary>
        public static MailTemplateTableRow Parse(string[] cells) => new()
        {
            MailTemplateTID = PackerUtil.ParseInt(cells[0], Table, "MailTemplateTID", 1, null),
            Title           = PackerUtil.RequireString(cells[1], Table, "Title"),
            Body            = PackerUtil.RequireString(cells[2], Table, "Body"),
        ...
        {
            var list = new List<MailTemplateTableRow>(rows.Count);
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<MailTemplateTableRow>>(bytes)
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<MailTemplateTableRow>>(bytes) ?? new();
