...
namespace GameData
{
    /// <summary>UnlockTable 시트의 셀 문자열을 UnlockTableRow로 변환하고 MemoryPack으로 직렬화한다. (패커 전용 — 클라/서버 배포 대상 아님)</summary>
    public static class UnlockTablePacker
    {
        private const string Table = "UnlockTable";
        ...
        {
            var list = new List<UnlockTableRow>(rows.Count);
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<UnlockTableRow>>(bytes)
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<UnlockTableRow>>(bytes) ?? new();
