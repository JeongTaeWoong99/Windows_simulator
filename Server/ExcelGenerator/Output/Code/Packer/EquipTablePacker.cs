...
namespace GameData
{
    /// <summary>EquipTable 시트의 셀 문자열을 EquipTableRow로 변환하고 MemoryPack으로 직렬화한다. (패커 전용 — 클라/서버 배포 대상 아님)</summary>
    public static class EquipTablePacker
    {
        private const string Table = "EquipTable";

        /// <summary>셀 배열(파싱된 컬럼 순서)을 강타입 Row로 변환한다.</summary>
        public static EquipTableRow Parse(string[] cells) => new()
        {
            EquipTID         = PackerUtil.ParseInt(cells[0], Table, "EquipTID", 1, null),
            ...
            EquipKind        = PackerUtil.ParseEnum<EquipKind>(cells[3], Table, "EquipKind"),
            Industry         = PackerUtil.ParseEnum<IndustryType>(cells[4], Table, "Industry"),
            SpeedAddPermille = PackerUtil.ParseInt(cells[5], Table, "SpeedAddPermille", -999, null),
            BasePrice        = PackerUtil.ParseInt(cells[6], Table, "BasePrice", 0, null),
        ...
        {
            var list = new List<EquipTableRow>(rows.Count);
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<EquipTableRow>>(bytes)
        ...
        {
            var list = MemoryPackSerializer.Deserialize<List<EquipTableRow>>(bytes) ?? new();
