...
**이 게임은 데스크톱 위에 상주하는 방치형이라 창이 배경에 얹혀 보여야 한다.** 그래서 기본이
보더리스(타이틀바 off)다 — 타이틀바가 남아 있으면 방치 화면이 그냥 "창에 든 게임"으로 보인다.

타이틀바를 지우면 **창을 옮길 손잡이도 같이 사라진다.** 대신 **메인 뷰 캔버스에 타이틀바 역할**을
주어(`WindowDragArea`) 잡아 끌면 창이 움직인다.
**창 이동은 빼도 되는 기능이 아니라, 타이틀바를 포기한 대가로 반드시 있어야 하는 기능이다.**
