...
public static class Win32Native
{
    ...
    // 현재 스레드의 활성 창 핸들을 반환. (스플래시 타이밍엔 부정확할 수 있어 폴백용으로만 사용)
    ...
    // 창의 현재 스타일 값을 읽기. (비트 제거/추가를 위해 먼저 현재 값을 읽을 때 사용)
    [DllImport("user32.dll")]
    ...
    // 창의 화면상 사각 영역(RECT)을 가져온다. 커서가 창 안 어디인지 계산할 때 사용.
    [DllImport("user32.dll")]
    ...
    // 창에 윈도우 메시지를 보낸다. 창 드래그(WM_SYSCOMMAND) 트리거에 사용.
    [DllImport("user32.dll")]
    ...
    // AdjustWindowRectExForDpi 의 dpi 없는 구형 버전. 폴백 전용.
    [DllImport("user32.dll")]
    ...
    // 모니터 핸들로 그 모니터의 전체/작업 영역 사각형을 가져온다.
    [DllImport("user32.dll")]
