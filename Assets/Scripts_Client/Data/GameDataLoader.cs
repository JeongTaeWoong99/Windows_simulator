...
public static class GameDataLoader
{
    ...
    public static void Load()
    {
        ...
        _isLoaded = true;
        Debug.Log($"[GameDataLoader] 테이블 적재 완료 — 아이템 {GameTable.ItemTable.Count}종, 캐릭터 {GameTable.CharacterTable.Count}종");
    }
    ...
    private static void WarnUnknownId(string kind, int id, HashSet<int> warned)
    {
        ...
        Debug.LogWarning($"[{nameof(GameDataLoader)}] {kind} 테이블에 없는 Id {id}가 들어왔다. " +
