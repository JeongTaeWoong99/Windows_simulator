...
> 최종 업데이트: 2026-08-23 (§7 — `SetActive`/`CanvasGroup` 판별 기준 승격, 로딩의 차단/표시 두 축) · 대상: `Assets/Scripts_Client/UI/`
...
| 종류 | 여닫는 법 | 무엇이 | 왜 |
|---|---|---|---|
| **밖에서 켜 주는 화면** | `SetActive` | 캔버스 · 메인 화면 셋 · 좌우 열 · 로그인 — **나머지 전부** | `UIManager`나 형제 Presenter가 켜 준다. 꺼져도 켜 줄 손이 살아 있고, `OnEnable` 재구독 + `Refresh`(§6)가 꺼진 동안 놓친 것을 따라잡는다 |
