...
> ⚠️ **탭 enum을 만들 때 값을 중간에 끼우지 않는다.** 씬에 int로 저장되므로 순서가 밀리면
> **배선이 조용히 어긋난다** — `MainScreen`에서 실제로 겪었다([`UIManager.cs`](../../Managers/UIManager.cs) 주석).
> 늘릴 때는 **뒤에만 붙인다.**

> 새 탭 Presenter는 `#Storage Canvas` 아래 **형제**로 두고 `LayoutElement`를
> `pref 0 · flexH 1`로 맞춘다. → 아직 구현 전이다(`tasks/T-036`).
...
**셋 다 화면을 만들 수 있는 상태가 아니다** — 캐릭터·장비·특성 모두 **데이터가 아직 안 온다.**
"UI만 만들면 된다"고 시작하면 중간에 그걸 발견한다.

**무엇을 기다리는지는 일감에 적혀 있다 → `tasks/T-036-창고탭전환.md`.**
