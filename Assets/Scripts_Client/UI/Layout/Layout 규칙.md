...
| **사람이 정하는 건 가운데 950 하나뿐** | 사이드를 인스펙터에서 고쳐도 다음 배치에서 덮어써진다 |

**`@Main Column`은 `flexibleHeight`를 전부 0으로 둔다** — 43 + 950 + 87 = 1080 = 컬럼 높이라
**남는 높이 자체가 없다.** 나눌 것이 없으면 사고도 없다.

## 파생값을 숫자로 베끼지 않는다

사이드 칸 높이(43·87)는 **가운데 950과 2 : 1 비율에서 나온 결과**다.
그 아래에 놓이는 것들도 이 숫자를 **손으로 적으면 안 된다** — 가운데나 비율이 움직일 때
따라가지 못하고 **조용히 어긋난다**(`WidgetPositionLayout`의 "이 값을 상수로 베껴 두지 말 것").

| 필요한 것 | 쓰는 것 |
|---|---|
| 높이만큼의 폭 (아이콘 등 정사각형) | [`SquareLayoutElement`](<../../Common/ugui-layout/ugui-layout 규칙.md>) |
| 위젯 칸 높이 (창 배율 역산) | `WidgetPositionLayout.WidgetSlotHeight` |

> 실제 사례 — 상태바 아이콘에 `LayoutElement.preferredWidth = 33`을 적었다가 걷어냈다(2026-09-02).
> 33은 `(1080 − 950) ÷ 3 − padding 10`의 결과였고, **가운데를 900으로만 바꿔도 직사각형이 됐다.**
