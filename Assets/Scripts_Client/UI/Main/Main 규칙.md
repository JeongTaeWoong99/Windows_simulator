...
> 최종 업데이트: 2026-09-24 (장비 장착 UI — T-074 · 산업 레벨 정보 펼침 · 화면의 스크롤을 하나로 — T-078) · 대상: `Assets/Scripts_Client/UI/Main/`
...
### 레벨 탭을 누르면 그 레벨의 정보가 펼쳐진다 (2026-09-24 · T-078)

`Industry Level Panel` 바로 아래 `Industry Level Info Panel`이 붙는다. 줄은 두 묶음이고 **프리팹이 다르다** —

| 묶음 | 줄 | 프리팹 |
|---|---|---|
| `■ 작업지 정보` | 기준 주기 · 판정당 경험치 | `EfficiencyRowView` (값 1칸) |
| `■ 나오는 자원` | 이름 · 확률 · 판매가 · 바탕은 등급 색 | **`IndustryDropRowView`** (값 2칸) |

- ⚠️ **자원 줄을 따로 만든 이유는 값이 둘이기 때문이다.** 한 칸에 `19.51% · 2,025 골드`로 붙여 쓰면
  **칸을 넘겨 잘린다**(2026-09-24 실측 — 120px 칸에서 잘렸다). 칸을 나눠 **확률 80 · 판매가 130** 고정폭을
  주면 잘리지 않고, 덤으로 **줄마다 세로 열이 맞는다**(`0.1%`와 `64.29%`가 한 칸에 있으면 어긋난다).
- 두 프리팹이 **부모를 나눠 쓰므로** 자원 줄은 켤 때마다 `SetAsLastSibling()`으로 뒤로 보낸다 —
  생성 순서에만 기대면 순서가 뒤집힐 수 있다.
- 풀은 효율 계산과 따로 둔다(둘이 동시에 보인다).
...
- ⚠️ 묶음 제목 줄은 등급 색을 쓰지 않으므로, 줄을 켤 때 **먼저 `Clear()`로 바탕색을 되돌린다** —
  안 되돌리면 앞서 자원 줄로 쓰이던 오브젝트의 등급 색을 제목이 뒤집어쓴다.

> **펼친 채로 시작하고, 고른 탭을 한 번 더 누르면 접힌다** (2026-09-24 사용자 결정).
> **높이를 정해 두지 않는다** — 화면이 한 스크롤이 되면서 내용만큼 늘어나게 됐다(실측 12줄 = 425).
> **펼침 표시는 고른 레벨의 버튼 라벨에 붙는다** — `Lv2 밭 ▲`(펼침) · `Lv2 밭 ▼`(접힘).
> 다른 버튼에는 붙이지 않는다 — 누르면 접힌다는 뜻이 되기 때문이다.
>
> ⚠️ **레벨 버튼에는 `LayoutElement`(`prefW 0 · flexW 1`)가 있어야 한다.** 없으면 `▲`가 붙은 버튼만
> 넓어진다 — 장비 칸이 `▼`에 밀려 1:1:1:1이 깨졌던 것과 **같은 함정**이다(T-074).
> ⚠️ 3단계에서 **같은 레벨을 다시 누르면 요청이 나가지 않는다**(`RequestIndustryLevelChange`의 조기 반환).
> 그래서 `SelectIndustryLevel`이 요청과 무관하게 버튼 갱신을 한 번 돌린다 — 안 그러면 접기가 화면에 안 보인다.
...
**고르는 동안에는 위아래를 모두 접는다** — 산업 레벨 정보 · `Character Label` · 캐릭터 카드 ·
...
- **닫으면 캐릭터 칸만 되돌린다.** ⚠️ **산업 레벨 정보는 접힌 채로 둔다** —
  고르려고 접은 것을 되살리면 **사용자가 스스로 접어 둔 것까지 덮어쓴다.**
  보고 싶으면 레벨 탭을 누르면 된다(그게 펼침 토글이다).
...
레벨 정보 · 캐릭터 목록 · 세팅 · 장비 목록이 **저마다 스크롤을 갖고 있었고 3중으로 겹쳤다.**
...
- **머리를 고정으로 남긴 이유** — 산업·레벨은 "지금 무엇을 보고 있나"의 표시이고,
  **레벨 정보를 접는 조작도 그 버튼에 있다.** 함께 굴러가면 접으려고 위로 되돌려야 한다.
...
- ⚠️ **줄을 그린 뒤 다시 재는 대상은 언제나 `bodyScroll.content`다.** 줄만 재면 스크롤 길이가
  한 프레임 늦어 끝이 잘려 보인다. 효율 줄도 레벨 정보 줄도 마찬가지다.
