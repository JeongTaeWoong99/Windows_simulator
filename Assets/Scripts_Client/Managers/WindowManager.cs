...
public class WindowManager : MonoService<WindowManager>
{
    ...
    // Topmost·크기·위치는 에디터(편집·플레이)에선 인스펙터('setStart*')가 진실이라 저장값을
    // 읽지 않는다. 값을 바꿔 바로 테스트할 수 있고, 옛 실행이 남긴 저장값이 인스펙터를 덮어써
    // "왜 안 바뀌지?"가 되는 것을 막는다('WidgetPositionLayout'과 같은 철학).
    // 빌드(.exe)에선 사용자 저장값('PlayerPrefs')이 진실이라 실행 간 유지된다(없으면 인스펙터 공장값).
    ...
    // 투명 배경 On/Off — DWM 프레임(유리)을 클라이언트 영역 전체로 확장/해제한다.
    ...
    // 항상 위 On/Off — Z순서를 HWND_TOPMOST/HWND_NOTOPMOST 로 바꾼다(위치·크기는 유지).
    ...
    // 동적 클릭 스루(마우스 위치 자동 판정)를 켜고 끈다.(디버그 패널 토글에서 호출)
...
#if !UNITY_EDITOR
    // 크기와 위치를 하나의 기준 모니터·단일 'SetWindowPos'로 원자 적용한다(A-1 근본 수정).
    //
    ...
    //   ② 위치 계산이 외곽 크기를 다시 추정해, 리사이즈의 실측 보정(dx/dy)과 어긋나
    //      오른쪽·아래 앵커가 프레임 두께만큼 넘쳤다.
    // → 작업 영역을 한 번만 고정하고, 실측 보정 시 위치까지 같은 외곽으로 재계산한다.
    //
    ...
    // 9분할 앵커의 외곽 좌상단 좌표를 작업 영역(wa)과 외곽 크기로 계산한다.
