...
public class ServerWaitManager : MonoService<ServerWaitManager>
{
    ...
    // 대기 중인 요청이 하나라도 있는가 — 로딩 표시의 on/off ('LoadingPresenter'가 구독).
    public event Action<bool>? BusyChanged;

    // 요청이 실패했거나 응답이 없었다 — 사용자에게 보일 문구 ('NoticePresenter'가 구독).
    public event Action<string>? NoticeRaised;

    // 연결 계층 치명 오류 — 알림 확인 후 종료로 이어진다 ('NoticePresenter'가 구독).
    ...
    //   onClosed : 대기가 끝날 때(성공·실패·타임아웃 공통) 호출 — 호출부의 버튼 잠금 해제에 쓴다
...
public sealed class ServerWaitHandle
{
    ...
    // 이미 끝난 대기인가 (성공·실패·타임아웃 중 하나로 닫혔다).
    ...
    // 정상 응답을 받았다 — 로딩을 조용히 내린다.
    public void Succeed() => _owner.Resolve(this, null);

    // 실패·오류 응답을 받았다 — 로딩을 내리고 알림 문구를 띄운다.
