...
> 최종 업데이트: 2026-08-28 (하트비트 — 주기 2초·백그라운드 타이머 송신) · 대상: `Assets/Scripts_Client/Managers/`
...
| **`PingManager`** | 연결 생존 확인(**2초 Ping** / 15초 무응답 감지). 송신은 백그라운드 타이머, 판정은 `Update` | **소켓을 끊지 않는다.** 알리고 앱을 내릴 뿐 — 세션 정리는 서버 폴더의 것 |
...
그래서 2초마다 Ping을 보내고 15초 무응답이면 `[연결]` 오류를 남긴 뒤,
...
### ⚠️ 핑 주기는 취향이 아니라 제약이다 — 서버 판정의 절반 이하

서버는 마지막 수신 후 `Global.SessionIdleTimeout`이 지나면 **로그인 여부와 무관하게** 세션을 끊는다.
주기를 그 값과 같게 두면 **한 번만 늦어도 끊긴다.**

실제로 판정 5초 · 주기 5초로 여유가 0이던 시기가 있었고(2026-08-28 원인 규명),
`_nextPingTime = now + 주기`가 매 프레임 오버슈트 몇 ms를 누적해 실제 간격이 5.008초쯤 되면서
**멀쩡한 세션이 수십 분에 한 번씩 무작위로 끊겼다.** "이유 없이 연결이 끊긴다"는 증상이 이것이었다.

주기를 **2초**로 내려 한 번 놓쳐도 살아남게 했다. 서버 판정을 15초로 되돌리는 일은
[`T-037`](../../../tasks/T-037-세션판정시간복구.md)(서버 담당)이다 — **그 뒤에도 2초는 그대로 유효하다.**

### ⚠️ 송신은 `Update`가 아니라 백그라운드 타이머가 한다

창을 잡아 끄는 동안 `WindowManager.BeginWindowDrag`가 창 이동을 **OS 모달 루프**에 위임한다
(5장 「창 이동은 캔버스 드래그로 OS에 위임한다」). 마우스를 놓을 때까지 **Unity 메인 루프가 통째로 멈춘다** —
`Update`에서 핑을 보내면 그동안 **한 개도 나가지 않아** 서버가 유휴 세션으로 보고 끊는다.
절전·긴 GC 히치도 같은 경로다.

그래서 `System.Threading.Timer`가 보낸다. 옮길 수 있는 이유는 **송신 경로가 이미 메인 스레드와
무관**하기 때문이다 — `MikaSendQueue`가 `ConcurrentQueue` 기반이고 실제 송신 루프
(`MikaClientSession.SendLoop`)도 스레드풀에서 돈다.

| 무엇 | 어디서 | 왜 |
|---|---|---|
| Ping 송신 | **타이머 스레드** | 메인 루프가 멈춰도 연결이 유지돼야 한다 |
| Pong 수신·무응답 판정·알림 | **메인 스레드(`Update`)** | 화면과 서비스를 만진다 |

세 가지를 함께 지킨다.

- **타이머 콜백에서 Unity API·`ClientLogger`를 부르지 않는다** — 로그는 `UnityEngine.Object`
  컨텍스트를 만진다. 예외는 삼켜 두었다가 메인 스레드가 대신 찍는다
  (타이머 콜백의 미처리 예외는 프로세스를 내린다).
- **시각은 `Stopwatch`로 잰다.** `Time.unscaledTime`은 메인 스레드 전용이고,
  `DateTime.UtcNow`는 시스템 시각 보정에 흔들린다.
- **`OnDisable`·`OnDestroy`에서 반드시 `Dispose`한다.** 안 하면 에디터에서 플레이를 멈춰도
  타이머 스레드가 도메인 리로드 너머까지 살아남는다.

> 🔑 **긴 프레임 정지 뒤의 첫 프레임에서는 무응답 판정을 건너뛴다.** 밀려 있던 Pong은
> `NetworkManager.Update`의 `Flush`가 처리하는데 **스크립트 실행 순서가 보장되지 않아**
> `PingManager.Update`가 먼저 돌 수 있다. 그대로 판정하면 멀쩡한 연결에 종료 알림이 뜬다.
> 프레임 간격이 1초를 넘으면 기준 시각만 다시 잡는다 — 진짜 끊김이면 다음 15초 안에 다시 잡힌다.
