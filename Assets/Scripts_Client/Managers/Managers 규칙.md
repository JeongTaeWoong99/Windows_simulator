...
> 최종 업데이트: 2026-09-18 (`FrameRateManager` 추가 — T-062) · 대상: `Assets/Scripts_Client/Managers/`
...
| **`FrameRateManager`** | 프레임 제한(30·60·90·144·모니터 동기화) 적용 · FPS 텍스트 위치를 정해 `FpsTextPositionChanged` 발행. **에디터에서도 저장값을 읽는다** ([`Settings 규칙.md`](<../Settings/Settings 규칙.md>) 1장) | FPS를 세거나 그리지 않는다 → `UI/System/FpsTextPresenter` |
| **`PingManager`** | 연결 생존 확인(5초 Ping / 15초 무응답 감지). **송신은 백그라운드 타이머, 판정은 `Update`** | **소켓을 끊지 않는다.** 알리고 앱을 내릴 뿐 — 세션 정리는 서버 폴더의 것 |
| **`SellCartModel`** | **판매하려고 담아 둔 목록.** 담긴 종류·수량, 합계 골드, 상위 등급 포함 여부를 들고 `Changed`를 발행. 창고 격자(담김 표시)와 `SellCartPresenter`(목록·합계)가 같은 것을 보게 하는 자리다 | **송신하지 않는다** — 판매 요청은 버튼을 누른 Presenter가 보낸다. **보유량의 주인도 아니다** — `PlayerDataModel.InventoryChanged`를 구독해 초과분만 깎는다 |
...
(5장 「창 이동은 캔버스 드래그로 OS에 위임한다」). 마우스를 놓을 때까지 **Unity 메인 루프가
통째로 멈춘다** — `Update`에서 보내면 그동안 핑이 **한 개도 나가지 않아** 서버가 유휴 세션으로
보고 끊는다. 절전·긴 GC 히치도 같은 경로다.

그래서 `System.Threading.Timer`가 보낸다.
...
> 🔑 **이 타이머는 혼자서는 듣지 않는다 — 서버 몫과 한 쌍이다.**
> `MikaClient`가 세션 루프를 `Task.Run` + `ConfigureAwait(false)`로 띄워
> **실제 송신 루프(`MikaClientSession.SendLoop`)가 메인 스레드 밖에서 돌아야** 큐가 비워진다
> (이슈 #19의 B-1, `add9b9f`). 그게 되돌아가면 **넣기만 하고 나가지 않아** 타이머도 무력해진다.
> 2026-08-28에 정확히 그 상태로 한 번 헛짚었다 — **넣는 쪽과 비우는 쪽 둘 다** 메인 스레드 밖이어야 한다.
...
> `PingManager.Update`가 먼저 돌 수 있다. 그대로 판정하면 **서버는 안 끊었는데 클라가
> 스스로 앱을 내린다.** 프레임 간격이 1초를 넘으면 기준 시각만 다시 잡는다 —
> 진짜 끊김이면 다음 15초 안에 다시 잡힌다.

### ⚠️ 핑 주기는 취향이 아니라 제약이다 — 서버 판정의 1/3 이하

서버는 마지막 수신 후 `Global.SessionIdleTimeout`(**15초**)이 지나면 **로그인 여부와 무관하게**
세션을 끊는다. 주기를 그 값과 같게 두면 **한 번만 늦어도 끊긴다.**

실제로 판정 5초 · 주기 5초로 여유가 0이던 시기가 있었고(2026-08-28 원인 규명 · 이슈 #19),
...
> 한쪽만 움직이면 여유가 다시 0이 된다. 기준은 **"핑 주기 ≤ 판정 시간 ÷ 3"** 이고,
> 지금 값(판정 15초 ÷ 3 = 주기 5초)이 그 경계에 정확히 맞춰져 있다.
> 부등식 셋은 [`작업슬롯`](../../../GameDesign/design/workslot/README.md) 3.3에 표로 있다.
...
**현재 이 폴더의 7개는 전부 1번(자기 자신)** 이다 — 교체 대상이 아니기 때문이다.
