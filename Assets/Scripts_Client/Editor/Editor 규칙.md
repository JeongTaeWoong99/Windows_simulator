...
- **버튼에 보이는 값은 프로세스의 워킹셋** — OS가 재는 "지금 물리 메모리에 올라가 있는 양"이다.
  아래 세 갈래를 더한 값이 **아니다**. 작업 관리자의 메모리 열은 프라이빗 작업 집합(공유 페이지 제외)이라
...
- **툴팁은 세 갈래로 쪼개 보여 준다** — 네이티브(에셋·씬) · Mono 힙(C# 스크립트) · 그래픽 드라이버.
  ...
  **이 셋의 합과 전체는 포함 관계가 아니다** — 전체에만 있는 몫(에디터 UI·플러그인·DLL 등 프로파일러가
  세지 않는 것)이 있는 반면, 예약분 중 물리에 안 올라간 것과 그래픽 드라이버의 VRAM 몫은 전체에서 빠진다.
  **합이 전체보다 클 수도 있다.**
