...
> 최종 업데이트: 2026-09-17 (버튼·알림 토글을 메인 툴바에서 치트 창 도구 줄로 이동) · 대상: `Assets/Scripts_Client/Editor/scene-copy/`

**오리지널 씬을 테스트용으로 복사하는 툴.** 폴더 전체에 걸리는 규칙(네임스페이스)은
[`Editor 규칙.md`](<../Editor 규칙.md>)에 있다. **버튼은 이 폴더에 없다** — 치트 창 도구 줄(`cheat-console/CheatWindow.cs`)에 있다.
...
- 진입점은 **치트 창 도구 줄의 `오리지널 씬 복사` 버튼** 하나뿐이다 — 메뉴 항목·단축키는 두지 않는다.
...
- **치트 창 도구 줄에서 `오리지널 씬 복사` 바로 앞의 `[●]`/`[○]`** 를 눌러 켜고 끈다(켜짐은 초록). 두 버튼을 붙여 `[●]오리지널 씬 복사`처럼 보이게 했다.
  창이 그릴 때마다 설정값을 읽으므로 환경 설정·팝업에서 바꿔도 표시가 따라온다.
- 낡음 팝업의 세 번째 버튼 **`[다시 알리지 않기]`**로 그 자리에서 끌 수 있다(다시 켜는 위치를 안내한다).
- 꺼지는 건 **자동 검사·팝업뿐**이다. 치트 창의 `오리지널 씬 복사` 버튼은 그대로 남아 언제든 수동 복사할 수 있다.
